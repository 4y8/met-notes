...
We give declarative rules for subtyping in Figure~\ref{decl-subt}. Like in \textsf{METL}, they
require an additional effect context. Contexts are those of \textsf{METL}.

\begin{figure}[h]
  \begin{mathpar}
    \inferrule[$\leq-$Var]
      {\Gamma\vdash\alpha:\textsf{Any}}
      {\Gamma\vdash\alpha\leq\alpha\ectx{E}}

    \inferrule[$\leq-$Unit]
      {\\}
      {\Gamma\vdash 1\leq1\ectx{E}}

    \inferrule[$\leq-$-Fun]
      {\Gamma\vdash B_1\leq A_1\ectx{E}\\
       \Gamma\vdash A_2\leq B_2\ectx{E}}
      {\Gamma\vdash A_1\to A_2\leq B_1\to B_2\ectx{E}}

    \inferrule[$\leq-$ForallL]
      {\Gamma\vdash\tau:K\\
       \Gamma\vdash[\tau/\alpha]A\leq B\ectx{E}}
      {\Gamma\vdash\forall\alpha:K.A\leq B\ectx{E}}

    \inferrule[$\leq-$ForallR]
      {\Gamma,\beta:K\vdash A\leq B\ectx{E}}
      {\Gamma\vdash A\leq\forall\beta.B\ectx{E}}

    \inferrule[$\leq-$Mod]
      {\Gamma\vdash(G,\overline{\mu})\Rightarrow\overline{\nu}\ectx{E}\\
       \Gamma\vdash G\leq G'\ectx{\overline{mu}(E)}}
      {\Gamma\vdash\overline{\mu} G\leq\overline{\nu} G\ectx{E}}
  \end{mathpar}

  \caption{Declarative subtyping}\label{decl-subt}
\end{figure}

\subsubsection{Declarative typing rules.}
The declarative typing rules are those of Dunfield and
Krishnaswami~\cite{dunfield2013complete} modified to take effect contexts into
account except for variables for which we use the \textsf{METL}. We add rules
for handlers, and the second rule to take modalities into account in
applications.

\subsubsection{Algorithmic contexts.}
...
\begin{figure}[h]
  ...
  \]
  \caption{Syntax of algorithmic contexts}\label{contexts}
\end{figure}


\subsubsection{Algorithmic subtyping and instantiation.}
We have the same algorithmic subtyping and instantiation rules to which we add
the rule \textsc{Sub-Mod} which deals with modality subtyping; there is no rule
for instantiation with modalities. And each rule has now an elaboration part
which is a (meta-theoretic) function, that maps terms to terms.
...
\begin{figure}[h]
  \centering
  \begin{mathpar}
    \inferrule[Sub-Var]{\\}{\Gamma[\alpha]\vdash\alpha\subt\alpha\dashv\Gamma[\alpha]\ectx{E}
      \elab{M\mapsto M}}
    
    \inferrule[Sub-ExVar]{\\}{\Gamma[\hat{\alpha}]\vdash\hat{\alpha}\subt\hat{\alpha}\dashv\Gamma[\hat{\alpha}]\ectx{E}
      \elab{M\mapsto M}}

    \inferrule[Sub-Unit]{\\}{\Gamma\vdash 1\subt1\dashv\Gamma\ectx{E}
      \elab{M\mapsto M}}

    \inferrule[Sub-Fun]{\Gamma\vdash B_1\subt A_1\dashv\Theta\ectx{E}
      \elab{f}\\
      \Theta{\vdash} [\Theta]A_2 {\subt} [\Theta]B_2\dashv\Delta\ectx{E}
      \elab{g}}{
    ...
    \inferrule[Sub-ForallL]{\Gamma,\blacktriangleright_{\hat{\alpha}},{\hat{\alpha}:K}
      {\vdash}[{\hat{\alpha}}/\alpha]A\subt B\dashv\Delta,\blacktriangleright_{\hat{\alpha}},\Theta\ectx{E}
      \elab{f}\\
      {\color{gray} \tau = [\Theta[\hat{\beta}=1/\hat{\beta}:K]]\hat{\alpha}}}
      {\Gamma\vdash\forall\alpha:K.A\subt B\dashv\Delta\ectx{E}
       \elab{M \mapsto f(M \tau)}}

  \inferrule[Sub-ForallR]{\Gamma,\alpha:K\vdash A\subt B\dashv\Delta,\alpha:K,\Theta\ectx{E}
    {\elab{f}}}
    {\Gamma\vdash A\subt \forall\alpha:K.B\dashv\Delta\ectx{E}
      \elab{M \mapsto \Lambda \alpha^K. f(M)}}

  \inferrule[Sub-InstantiateL]{\hat{\alpha}\not\in\textsf{FV}(A)\\
    {\Gamma}[\hat{\alpha}]\vdash\hat{\alpha}:\leqq A\dashv\Delta\ectx{E}
    \elab{f}}
    {{\Gamma}[\hat{\alpha}]\vdash\hat{\alpha}\subt A\dashv\Delta\ectx{E}
      \elab{f}}

  \inferrule[Sub-InstantiateR]{\hat{\alpha}\not\in\textsf{FV}(A)\\
    {\Gamma}[\hat{\alpha}]\vdash A\leqq:\hat{\alpha}\dashv\Delta\ectx{E}
    \elab{f}}
    {{\Gamma}[\hat{\alpha}]\vdash A\subt \hat{\alpha}\dashv\Delta\ectx{E}
      \elab{f}}

  \inferrule[Sub-Mod]
  {\Gamma\vdash(G,\overline{\mu})\Rightarrow \overline{\nu}\ectx{E}\\
   \Gamma\vdash G\subt G'\dashv\Delta\ectx{\overline{\mu}(E)}
   \elab{f}\\
  }
  {\Gamma\vdash\overline{\mu}G\subt\overline{\nu}G'\dashv\Delta\ectx{E}
  \elab{M\mapsto\textsf{\textbf{let mod}}_{\overline{\mu}} x = M \textsf{\textbf{ in }} \kmod_{\overline{\nu}} f(x)}}
  \end{mathpar}
...
\begin{figure}[h]
  \begin{mathpar}
    \inferrule[InstL-Solve]{\Gamma\vdash\tau:K}
      {\Gamma,\hat{\alpha}:K,\Gamma'\vdash\hat{\alpha}:\leqq\tau\dashv\Gamma,\hat{\alpha}=\tau,\Gamma'\ectx{E}
      \elab{M\mapsto M }}

    \inferrule[InstL-Reach]{\\}
      {{\Gamma}[\hat{\alpha}:K][\hat{\beta}:K'] \vdash\hat{\alpha}:\leqq\hat{\beta}\dashv\Gamma[\hat{\alpha}:K\wedge K'][\hat{\beta}=\hat{\alpha}]\ectx{E}
        \elab{M\mapsto M}}

    \inferrule[InstL-Arr]
      {{\Gamma}[{\hat{\alpha}}_2 :\textsf{Any}, {\hat{\alpha}}_1 :\textsf{Any},
                \hat{\alpha}={\hat{\alpha}}_1\to{\hat{\alpha}}_2]
        \vdash A_1\leqq: {\hat{\alpha}}_1 \dashv\Theta\ectx{E}
        \elab{f}\\
        \Theta\vdash {\hat{\alpha}}_2:{\leqq}[\Theta]A_2\dashv\Delta\ectx{E}
        \elab{g}}
      {{\Gamma}[{\hat{\alpha}:\textsf{Any}}]\vdash\hat{\alpha}:\leqq A_1 \to A_2\dashv\Delta\ectx{E}
        \elab{M\mapsto\lambda x^{A_1}.g (M f(x))}}

    \inferrule[InstL-AllR]
      {{\Gamma}[\hat{\alpha}],\beta:K\vdash\hat{\alpha}:\leqq B\dashv\Delta,\beta:K,\Delta'\ectx{E}
        \elab{f}}
      {{\Gamma}[\hat{\alpha}]\vdash\hat{\alpha}:\leqq \forall\beta:K.B\dashv\Delta\ectx{E}
        \elab{M\mapsto\Lambda\beta^{K}.f(M)}}
  \end{mathpar}

\begin{mathpar}
    \inferrule[InstR-Solve]{\Gamma\vdash\tau:K}
      {\Gamma,\hat{\alpha}:K,\Gamma'\vdash\tau\leqq:\hat{\alpha}\dashv\Gamma,\hat{\alpha}=\tau,\Gamma'\ectx{E}
      \elab{M\mapsto M}}

    \inferrule[InstR-Reach]{\\}
      {{\Gamma}[\hat{\alpha}:K][\hat{\beta}:K'] \vdash\hat{\beta}\leqq:\hat{\alpha}\dashv\Gamma[\hat{\alpha}:K\wedge K'][\hat{\beta}=\hat{\alpha}]\ectx{E}
        \elab{M\mapsto M}}

    \inferrule[InstR-Arr]
      {{\Gamma}[{\hat{\alpha}}_2:\textsf{Any}, {\hat{\alpha}}_1 : \textsf{Any},
                 \hat{\alpha}={\hat{\alpha}}_1\to{\hat{\alpha}}_2]
        \vdash {\hat{\alpha}}_1:\leqq A_1 \dashv\Theta\ectx{E}
        \elab{f}\\
        \Theta\vdash [\Theta]A_2{\leqq}:{\hat{\alpha}}_2\dashv\Delta\ectx{E}
        \elab{g}}
      {{\Gamma}[{\hat{\alpha}}:\textsf{Any}]\vdash A_1 \to A_2\leqq:\hat{\alpha}\dashv\Delta\ectx{E}
        \elab{M\mapsto\lambda x^{{\hat{\alpha}}_1}.g (M f(x))}}

    \inferrule[InstR-AllL]
      {{\Gamma}[{\hat{\alpha}}],\blacktriangleright_{\hat{\beta}},{\hat{\beta}:K}
      {\vdash}[{\hat{\beta}}/\beta]B\leqq: \hat{\alpha}\dashv\Delta,\blacktriangleright_{\hat{\beta}},\Delta'\ectx{E}
      \elab{f}\\
      {\color{gray} \tau = [[\hat{\gamma}=1/\hat{\gamma}:K]\Delta']\hat{\beta}}}
      {{\Gamma}[\hat{\alpha}]\vdash\forall\beta:K.B\leqq:\hat{\alpha}\dashv\Delta\ectx{E}
        \elab{M\mapsto f(M\tau)}}
  \end{mathpar}
...
\inferrule[B-Switch]
  {\Gamma\vdash M\Rightarrow A\dashv\Theta\elab{M'}\\
    \Theta{\vdash}[\Theta]A{\subt}[\Theta]B\dashv\Delta\ectx{E}\elab{f}\\
    \text{no other rule apply}
  }{\Gamma\vdash M\Leftarrow B\dashv\Delta\ectx{E}\elab{f(M')}}
